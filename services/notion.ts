...
async function retrieveBlockChildren(id: any) {
...
export async function getNotionPageContent(pageId: string) {
  ...
  const content = printBlockText(blocks);
  return content;
}

export const getSubPages = async (pageId: string) => {
  const blocks = await retrieveBlockChildren(pageId);
  const subPages = blocks.filter((block: any) => block.type === "child_page").map((block: any) => ({
